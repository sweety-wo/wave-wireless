...
export interface Statement { 
    ...
    id?: string;
    /**
     * The resulting effect that should be applied as a result of the statement. Allowed values are `ALLOW` or `DENY`.  This allows both implicit and explicit denial of services.
     */
    effect?: string;
    /**
     * A collection of service context actions that must be matched for the statement to take effect. The action keys are used to manager application action privileges. The actions are designed to format the action request designating the target application, service and action in the following format: [application]:[service]:[action]. The application and service are always in lower case while the action is in Camel Case. e.g. access:group:AddAccount. For more information, see the documentation.
     */
    actions?: Array<string>;
    /**
     * An optional expression. If this value is supplied access to an application or service will be considered conditional. The role will perform evaluations on an individual model basis to determine whether they pass or fail the expression.
     */
